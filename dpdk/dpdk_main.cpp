...
int net_device_transmit(struct net_device *dev, uint8_t *buffer,
                        size_t len);  // 宣言のみ
...
static inline int port_init(uint16_t port, struct rte_mempool *mbuf_pool) {
    ...
    sprintf(dev->name, "dpdk%d",
            port); // net_deviceにインターフェース名(dpdkX)をセット
    memcpy(dev->mac_addr, addr.addr_bytes,
           6); // net_deviceにMACアドレスをセット
    ((net_device_data *)dev->data)->port =
        port; // プラットフォーム依存データをセットｓ
...
static __rte_noreturn void lcore_main(void) {
    ...
    configure_ip_address(get_net_device_by_name("dpdk0"),
                         IP_ADDRESS(10, 0, 0, 2), IP_ADDRESS(255, 255, 255, 0));
    configure_ip_address(get_net_device_by_name("dpdk1"),
                         IP_ADDRESS(10, 0, 1, 2), IP_ADDRESS(255, 255, 255, 0));
