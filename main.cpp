...
void configure() {
    ...
    */
    ...
    IP_ADDRESS(192, 168, 0, 1), IP_ADDRESS(255, 255, 255, 0));
    configure_ip_address(get_net_device_by_name("router1-router2"),
    IP_ADDRESS(192, 168, 1, 1), IP_ADDRESS(255, 255, 255, 0));
    configure_ip_address(get_net_device_by_name("router1-router4"),
    IP_ADDRESS(192, 168, 4, 2), IP_ADDRESS(255, 255, 255, 0));
    //configure_net_route(IP_ADDRESS(192, 168, 5, 0), 24, IP_ADDRESS(192, 168,
    1, 2)); configure_ip_net_route(IP_ADDRESS(192, 168, 5, 0), 24,
    ...
    */
    ...
    IP_ADDRESS(192, 168, 3, 1), IP_ADDRESS(255, 255, 255, 0));
    configure_ip_net_route(IP_ADDRESS(192, 168, 2, 0), 24, IP_ADDRESS(192, 168,
    0, 2)); configure_ip_net_route(IP_ADDRESS(192, 168, 4, 0), 24,
    ...
    for (ifaddrs *tmp = addrs; tmp; tmp = tmp->ifa_next) {
        if (tmp->ifa_addr && tmp->ifa_addr->sa_family == AF_PACKET) {
            ...
            auto *dev = (net_device *)calloc(
                ...
                sizeof(net_device) +
                    sizeof(
                        net_device_data)); // net_deviceの領域と、net_device_dataの領域を確保する
            dev->ops.transmit = net_device_transmit; // 送信用の関数を設定
            dev->ops.poll = net_device_poll; // 受信用の関数を設定

            strcpy(dev->name,
                   tmp->ifa_name); // net_deviceにインターフェース名をセット
            ...
            fcntl(sock, F_SETFL,
                  val | O_NONBLOCK); // Non blockingのビットをセット
        }
    }
    // 確保されていたメモリを解放
    freeifaddrs(addrs);
