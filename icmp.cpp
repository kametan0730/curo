    ...
    switch (icmp_msg->header.type) {
        case ICMP_TYPE_ECHO_REPLY:
            ...
            LOG_ICMP("Received icmp echo reply id %04x seq %d\n",
                     ntohs(icmp_msg->echo.identify),
                     ntohs(icmp_msg->echo.sequence));
            break;
...
void send_icmp_time_exceeded(uint32_t dest_addr, uint32_t src_addr,
    ...
    my_buf *time_exceeded_mybuf =
        my_buf::create(sizeof(icmp_header) + sizeof(icmp_time_exceeded) +
                       sizeof(ip_header) + 8);
...
void send_icmp_destination_unreachable(uint32_t dest_addr, uint32_t src_addr,
    ...
    // ICMPヘッダ+メッセージの領域+エラーパケット部(IPヘッダ+バイト)を確保
    my_buf *unreachable_mybuf =
        my_buf::create(sizeof(icmp_header) + sizeof(icmp_dest_unreachable) +
                       sizeof(ip_header) + 8);
    ...
    unreachable_msg->dest_unreachable.unused = 0;
    memcpy(unreachable_msg->dest_unreachable.data, error_ip_buffer,
           sizeof(ip_header) + 8);
