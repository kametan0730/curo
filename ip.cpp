        ...
        if (current_node->data != nullptr) {
            if (current_node->data->type == ip_route_type::connected) {
                printf("%s/%d connected %s\n",
                       ip_htoa(locate_prefix(current_node, ip_fib)),
                       current_node->depth, current_node->data->dev->name);
            } else {
...
void ip_input_to_ours(net_device *input_dev, ip_header *ip_packet, size_t len) {
    ...
    for (net_device *dev = net_dev_list; dev; dev = dev->next) {
        if (dev->ip_dev != nullptr and dev->ip_dev->nat_dev != nullptr and
            ...
            if (nat_executed) {
                ...
                nat_fwd_mybuf->len = len;
#endif
                ip_output(ntohl(ip_packet->dest_addr),
                          ntohl(ip_packet->src_addr), nat_fwd_mybuf);
                return;
            }
        }
    }
    ...
    switch (ip_packet->protocol) {
        case IP_PROTOCOL_NUM_ICMP:
            ...
            */
            return icmp_input(
                ntohl(ip_packet->src_addr), ntohl(ip_packet->dest_addr),
        ...
        case IP_PROTOCOL_NUM_UDP:
            ...
            send_icmp_destination_unreachable(
                ...
                ICMP_DEST_UNREACHABLE_CODE_PORT_UNREACHABLE, ip_packet, len);
#endif
            return;
        case IP_PROTOCOL_NUM_TCP:
...
void ip_input(net_device *input_dev, uint8_t *buffer, ssize_t len) {
    ...
    if (ip_packet->dest_addr ==
        IP_ADDRESS_LIMITED_BROADCAST) { // 宛先アドレスがブロードキャストアドレスの場合
        return ip_input_to_ours(input_dev, ip_packet,
                                len); // 自分宛の通信として処理
    }
    ...
    for (net_device *dev = net_dev_list; dev; dev = dev->next) {
        if (dev->ip_dev != nullptr and
            ...
            // 宛先IPアドレスがルータの持っているIPアドレス or
            // ディレクティッド・ブロードキャストアドレスの時の処理
    ...
    ip_route_entry *route = binary_trie_search(
        ip_fib,
        ntohl(ip_packet->dest_addr)); // ルーティングテーブルをルックアップ
    ...
    if (route->type == connected) { // 直接接続ネットワークの経路なら
        ip_output_to_host(route->dev, ntohl(ip_packet->dest_addr),
                          ...
                          ip_fwd_mybuf); // hostに直接送信
        return;
    } else if (route->type ==
               network) { // 直接接続ネットワークの経路ではなかったら
...
void ip_output_to_host(net_device *dev, uint32_t dest_addr, uint32_t src_addr,
                       my_buf *payload_mybuf) {
    arp_table_entry *entry =
        search_arp_table_entry(dest_addr); // ARPテーブルの検索
    ...
    if (!entry) { // ARPエントリが無かったら
        ...
        ethernet_encapsulate_output(
            entry->dev, entry->mac_addr, payload_mybuf,
            ETHER_TYPE_IP); // イーサネットでカプセル化して送信
    }
}
...
void ip_output_to_next_hop(uint32_t next_hop, my_buf *payload_mybuf) {
    arp_table_entry *entry =
        search_arp_table_entry(next_hop); // ARPテーブルの検索

    if (!entry) { // ARPエントリが無かったら
        LOG_IP("Trying ip output to next hop, but no arp record to %s\n",
               ip_htoa(next_hop));

        ip_route_entry *route_to_next_hop = binary_trie_search(
            ip_fib, next_hop); // ルーティングテーブルのルックアップ

        if (route_to_next_hop == nullptr or
            route_to_next_hop->type !=
                connected) { // next hopへの到達性が無かったら
            LOG_IP("Next hop %s is not reachable\n", ip_htoa(next_hop));
        } else {
            send_arp_request(route_to_next_hop->dev,
                             next_hop); // ARPリクエストを送信
        }
        my_buf::my_buf_free(payload_mybuf, true); // Drop packet
        return;
...
void ip_encapsulate_output(uint32_t dest_addr, uint32_t src_addr,
    ...
    /*
    // for book chapter3 (IP
    ルーティング/フォワーディングが実装されてないとき用) for(net_device* dev =
    net_dev_list; dev; dev = dev->next){ if(dev->ip_dev == nullptr or
    dev->ip_dev->address == IP_ADDRESS(0, 0, 0, 0)) continue;
    ...
    ip_htoa(dest_addr)); send_arp_request(dev, dest_addr);
